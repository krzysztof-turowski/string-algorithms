...
class wavelet_tree:
  def __init__(self, t, n, sorted_alphabet_list = None):
    ...
    for i in range(n):
      self.left_indexes.append(i+1) if t[i] in self.zero_indexed else self.rigth_indexes.append(i+1)
    left_node_text = '#' + ''.join(c for c in t if c in self.zero_indexed)
    rigth_node_text = '#' + ''.join(c for c in t if c in self.one_indexed)
    self.left = wavelet_tree(left_node_text, len(left_node_text) - 1, left_alphabet)
    self.right = wavelet_tree(rigth_node_text, len(rigth_node_text) - 1, right_alphabet)

  def _left_tree_range(self, l, r):
    return l - self.prefix_sum[l-1], r - self.prefix_sum[r]
    
  def _right_tree_range(self, l, r):
    return self.prefix_sum[l] + (1 if self.t[l] in self.zero_indexed else 0), self.prefix_sum[r]
  ...
  def select(self, c, k, l, r):
    ...
    if c in self.zero_indexed:
      new_l, new_r =  self._left_tree_range(l, r)
      recursion_result = self.left.select(c, k, new_l, new_r)
      return self.left_indexes[recursion_result] if recursion_result is not None else None
    new_l, new_r =  self._right_tree_range(l, r)
    recursion_result = self.right.select(c, k, new_l, new_r)
    return self.rigth_indexes[recursion_result] if recursion_result is not None else None
  ...
  def _does_one_range_end_in_another(self, l, r, i, j):
    return (i <= l and j >= l) or (i <= r and j >= r)
  ...
  def range_count(self, l, r, x, y):
    if l > r or l > self.n or l < 1 or x > y:
      return 0
    if y < self.smallest or x > self.biggest:
      return 0
    if x <= self.smallest  and self.biggest <= y:
      return r-l+1
    if self.leaf:
      return 0
    if self._ranges_intersect(self.left.smallest, self.left.biggest, x, y) and \
      self._ranges_intersect(self.right.smallest, self.right.biggest, x, y): 
