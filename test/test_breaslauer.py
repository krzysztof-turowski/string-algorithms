...
from shortest_common_superstring import breaslauer, common, kosaraju
...
class TestOmegaRotation(unittest.TestCase):
  ...
  def check_overlap_rotation_lemma(self, W, candidates):
    rotation = breaslauer.get_omega_rotation(W)
    period_alpha = infinite_period(W)
    long_repeat = rotation * (max(1, max(len(c) for c in candidates) // len(W) + 3))
    for s in candidates:
      ...
      if period_s <= period_alpha:
        self.assertLess(ov, (2 / 3) * (period_s + period_alpha) + 1e-9,
                         f'(stronger bound) W={W}, ->W={rotation}, s={s}, ov={ov}')

  def test_handmade_periodic_strings(self):
    self.check_overlap_rotation_lemma('ab', ['aab', 'bba', 'ba', 'abb', 'a', 'b'])
    self.check_overlap_rotation_lemma('abc', ['cab', 'bca', 'abcab', 'ccc', 'aabbcc'])
  ...
  def test_random_periodic_strings(self):
    ...
    for _ in range(200):
      ...
      candidates = [''.join(rng.choice(alphabet) for _ in range(rng.randint(1, 12)))
                    for _ in range(15)]
  ...
  def test_random_periodic_strings_large_alphabet(self):
    ...
    for _ in range(2000):
      ...
      W = ''.join(rng.choice(alphabet) for _ in range(period_len))
      candidates = [''.join(rng.choice(alphabet) for _ in range(rng.randint(1, 20)))
                    for _ in range(20)]
      self.check_overlap_rotation_lemma(W, candidates)
...
class TestConstructTc(unittest.TestCase):
  ...
  def check_lemma_5_1(self, strings, cycle_indices):
    L = len(cycle_indices)
    t_c = breaslauer.construct_tc(cycle_indices, strings)

    broken = [breaslauer.build_broken_cycle_string(
        [cycle_indices[(bp + i) % L] for i in range(L)], strings)
        for bp in range(L)]

    found_i = any(broken[j] in t_c for j in range(L))
    self.assertTrue(found_i,
                     f'Lemma 5.1 condition (i) not satisfied: t_c={t_c!r}, broken={broken!r}')

    W = ""
    for i in range(L):
      u = strings[cycle_indices[i]]
      v = strings[cycle_indices[(i + 1) % L]]
      ov = breaslauer.get_overlap(u, v)
      W += u[:-ov] if ov > 0 else u
    omega = breaslauer.get_omega_rotation(W)
    ...
    self.assertIsNotNone(winner_bp, f'No break point found in ->omega^inf: {broken!r}')

    j = (winner_bp - 1) % L
    container_seq = [cycle_indices[(j + i) % L] for i in range(L)] + [cycle_indices[j]]
    container = breaslauer.build_broken_cycle_string(container_seq, strings)
    self.assertIn(t_c, container,
                   f'Lemma 5.1 condition (ii) not satisfied: t_c={t_c!r}, container={container!r}')
  ...
  def test_random_cycles(self):
    ...
    for _ in range(200):
      ...
      strings = [''.join(rng.choice(alphabet) for _ in range(rng.randint(2, 8)))
                 for _ in range(k)]
      strings = breaslauer.remove_substrings(strings)
      if len(strings) < 2:
        continue
...
class TestOverlapAlgorithmParameter(unittest.TestCase):
  ...
  def test_default_matches_kosaraju_superstring(self):
    strings = ['#abc', '#bcf', '#fgh']
    result = breaslauer.breslauer_jiang_jiang_2_596(strings)
    for s in strings:
  ...
  def test_custom_overlap_algorithm_is_actually_invoked(self):
    ...
    strings = ['#abc', '#bcf', '#fgh']
    breaslauer.breslauer_jiang_jiang_2_596(strings, overlap_algorithm=spy_algorithm)
    self.assertEqual(len(calls), 1, 'overlap_algorithm should be called exactly once')

  def test_custom_overlap_algorithm_result_is_used(self):
    marker = 'THIS_IS_A_MARKER_VALUE'
    strings = ['#abc', '#bcf', '#fgh']
    result = breaslauer.breslauer_jiang_jiang_2_596(strings, overlap_algorithm=lambda T: marker)
  ...
  def test_random_custom_overlap_algorithm(self):
    ...
    for _ in range(50):
      ...
      strings = ['#' + ''.join(rng.choice(alphabet) for _ in range(rng.randint(3, 8)))
                 for _ in range(k)]
      result = breaslauer.breslauer_jiang_jiang_2_596(
