...
def get_word_to_mid_lcp(SA, text, word, mid, word_to_edge_lcp, edge_to_mid_lcp):
  if edge_to_mid_lcp >= word_to_edge_lcp:
    word_to_mid_lcp = word_to_edge_lcp + get_longest_common_prefix(text[SA[mid] + word_to_edge_lcp:], word[1 + word_to_edge_lcp:])
  else:
...
def right_bound_with_lcplr(SA, lcplr, text, word, n, m):
  ...
  while left + 1 < right:
    mid = (left + right) // 2
    if l >= r:
      word_to_mid_lcp = get_word_to_mid_lcp(SA, text, word, mid, word_to_edge_lcp = l, edge_to_mid_lcp = lcplr[(left, mid)])
    else:
      word_to_mid_lcp = get_word_to_mid_lcp(SA, text, word, mid, word_to_edge_lcp = r, edge_to_mid_lcp = lcplr[(mid, right)])
    
    if word_to_mid_lcp == m or word[1 + word_to_mid_lcp] > text[SA[mid] + word_to_mid_lcp]:
