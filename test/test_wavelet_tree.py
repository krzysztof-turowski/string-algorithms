...
class TestWaveletTree(unittest.TestCase):
  ...
  model_solver = type(dummy_solver)

  test_classes = [wavelet_tree.wavelet_tree]
  ...
  def tree_api_random_test(self, n, q, alphabet):
    ...
    model_solver = dummy_solver(text, n)
    runners_and_queries = [(runner, self.create_queries(n, q, alphabet, fun)) for (fun, runner) in self.runner_functions]
    model_results = [runner(model_solver, queries) for (runner, queries) in runners_and_queries]
    for cls in self.test_classes:
      solver = cls(text, n)
      results =  [runner(solver, queries) for (runner, queries) in runners_and_queries]
