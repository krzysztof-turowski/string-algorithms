...
def counting_sort(A, ind):
    n = max(max(A, key=itemgetter(0))[0], max(A, key=itemgetter(1))[1], len(A)) + 1
    result = [0] * n
    count = [0] * n
    for tp in A:
        count[tp[ind]] += 1
...
# for every node c that has both odd and even descendants, find pair (a,b) such that a and b are descendants of c,
...
# merge odd and even suffix arrays, using longest common prefix between odd and even suffixes
# longest common prefix of odd and even suffix is computed as lca query on overmerged odd and even trees
...
# append symbol as many times as necessary to make the text length equal some closest power of two greater than original
# length
...
# runs Farach's algorithm on extended text and then discards extra nodes / array entries related to extra symbols added
# at the end of text; finally returns these structures for the original text
# handles both normal strings (starting with '#') and strings over integer alphabet
...
# main function in Farach's algorithm; builds odd, even suffix trees and merges them into correct suffix tree of the
# entire text
...
# recursive method called for every node, used to create overmerge odd and even suffix trees
# preserves node indices from odd and even trees; adds other indices to other nodes
# this way, if some node nd has index i from 1 to len(text), it means that word(nd) is a suffix of text starting at i
def get_faulty_merge_recurse(node, odd, even, text):
    ...
    i = 0
    j = 0
    # two following lines won't work linear, but for clarity, they are written this way
    # Farach's algorithm works on lists which are already sorted in odd, even and resulting tree, but current API uses
    # dictionary to represent children set and hence the call to 'sorted' function; if we used list instead of dict,
...
# algorithm implemented based on https://en.wikipedia.org/wiki/LCP_array#Suffix_tree_construction
def suffix_and_LCP_array_to_tree(A, LCP, text):
    ...
    for i in range(1, n):
        ...
        if currentNode.depth == LCP[i - 1]:
            ...
            rightmostChild = sorted(currentNode.children.items())[len(currentNode.children) - 1][1]
            splitNode = suffix_tree.break_node(currentNode, rightmostChild, LCP[i - 1] - currentNode.depth)
...
def get_odd_suffix_tree(text):
    ...
    for i in range(n // 2 - 1):
        if text[A_To[i] - 1 + 2 * LCP_TS[i]] == text[A_To[i + 1] - 1 + 2 * LCP_TS[i]]:
...
# returns suffix tree for even suffixes of text based on already created odd suffix tree and odd suffix array
