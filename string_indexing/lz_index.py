...
class _LZTreeNode:
  def __init__(self, parent, character, index, position):
    self.parent = parent
  ...
  def set_ranks(self, rank):
    if self.index is not None:
      self.rank, self.left_rank, self.right_rank = rank, rank, rank
      rank = rank + 1
    if len(self.children) > 0:
      for child_key in sorted(self.children):
        rank = self.children[child_key].set_ranks(rank)
      min_key, max_key = min(self.children), max(self.children)
      self.left_rank = (
        self.children[min_key].left_rank
        if (self.rank is None or self.children[min_key].left_rank < self.rank)
        else self.rank)
      self.right_rank = (
        self.children[max_key].right_rank
...
def _search_internal(t, index, n, node):
  if index == n:
    return node
  if t[index + 1] not in node.children:
    return None
  return _search_internal(t, index + 1, n, node.children[t[index + 1]])
  ...
  def __init__(self, t, n):
    ...
    for i in range(1, n + 2):
      ...
      if current_char not in current_node.children:
        _LZTreeNode(current_node, current_char, index, position)
        index += 1
        current_node = self.root
        position = i + 1
      else:
...
class _RangeSearcher:
  ...
  def search_in_range(self, l1, r1, l2, r2):
    ...
    if left > right or left == len(self.points) or right == -1:
      return []
    return [
      self.points[x - 1] for x in self.wavelet_tree.range_search(
        left + 1, right + 1, l2, r2)]
...
class _RevLZTrie:
  ...
  def _add_recursive(self, node):
    for child in node.children.values():
      ...
      self._add_block(child, self.root, child.index)

  def _add_block(self, lz_node, rev_node, index):
    if lz_node.parent is None or lz_node.parent.character == '#':
      if lz_node.character in rev_node.children:
        rev_node.children[lz_node.character].index = index
      else:
        rev_node.children[lz_node.character] = _LZTreeNode(
          rev_node, lz_node.character, index, None)
    else:
      if lz_node.character not in rev_node.children:
        rev_node.children[lz_node.character] = _LZTreeNode(
          rev_node, lz_node.character, None, None)
      self._add_block(
        lz_node.parent, rev_node.children[lz_node.character], index)

#pylint: disable=too-few-public-methods
class _LZIndex:
  def __init__(
      self, lz_trie, rev_lz_trie, lz_node_mapper, rev_lz_node_mapper,
...
def _contains_internal(lz_index : _LZIndex, s, m):
  yield from _contains_in_single_block(lz_index, s, m)
  yield from _contains_within_two_blocks(lz_index, s, m)
  yield from _contains_within_three_or_more_blocks(lz_index, s, m)
...
def _contains_in_single_block(lz_index : _LZIndex, s, m):
  ...
  if root is not None:
    for i in range(root.left_rank, root.right_rank + 1):
      ...
      node = lz_index.lz_node_mapper.get_node_by_index(rev_node.index)
  ...
  for i in range(1, m+1):
    ...
    for j in range(i, m+1):
      ...
      if current_node is not None:
        recorded[current_node.index] = j
    arr.append(recorded)
  return used, existance, arr

def _contains_within_three_or_more_blocks(lz_index : _LZIndex, s, m):
  used, existance, array = _prepare_structures_for_third_case(lz_index, s, m)
  for i in range(1, m + 1):
    for j in range(i, m + 1):
      if existance[i][j] is None or used[i][j] is True:
        continue
      start_index = existance[i][j].index
      current_index, current_end = start_index, j
      while current_end < m and (current_index + 1) in array[current_end+1]:
        current_index = current_index + 1
        used[current_end + 1][array[current_end + 1][current_index]] = True
        current_end = array[current_end + 1][current_index]
      ...
      if size < 3 or (current_end != m and existance[current_end+1][m] is None):
        continue
      if (lz_index.lz_trie.size > current_index + 1 and (current_end == m or
          (existance[current_end+1][m].left_rank <=
          lz_index.lz_node_mapper.get_node_by_index(current_index + 1).rank <=
          existance[current_end+1][m].right_rank ))):
        if i == 1:
          yield lz_index.lz_node_mapper.get_node_by_index(start_index).position
          continue
        if start_index == 1:
          continue
        current_node = lz_index.lz_node_mapper.get_node_by_index(
          start_index - 1)
        previous = i - 1
        while (previous > 0 and current_node.parent is not None and
            s[previous] in current_node.parent.children and
            current_node.parent.children[s[previous]] == current_node):
          previous -= 1
          current_node = current_node.parent
        if previous == 0:
          node = lz_index.lz_node_mapper.get_node_by_index(start_index)
...
def create_lz_index(t, n):
  ...
  points = [(
    rev_node_mapper.get_node_by_index(i).rank,
    lz_node_mapper.get_node_by_index(i + 1).rank)
  ...
  return _LZIndex(
    lz_trie, rev_trie, lz_node_mapper, rev_node_mapper,
    range_searcher, lz_rank_mapper, rev_lz_rank_mapper)
